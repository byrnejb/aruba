...
Aruba provides also support for testing using various Ruby versions (see below).
...
directories.  A string of directories to rebase may be passed by setting
the ARUBA_REBASE environmental variable.  These must be set relative
...
the <tt>rebase</tt> method also is deactivated.
...
Aruba steps more specifis so as to avoid anticipate collisions
with the user's own steps.  On the other hand, most Aruba steps
are now defined as sub strings so that the subject is no longer
part of the matcher.  For example, instead of:
...
exercised features not using JRuby pass when exercised using 
<tt>cucumber features</tt>.  
